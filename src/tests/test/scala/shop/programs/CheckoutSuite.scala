...
import shop.generators.userIdGen
import shop.http.clients.PaymentClient
import shop.domain.order.{EmptyCartError, Order, OrderError, OrderId, PaymentError, PaymentId}
...
object CheckoutSuite extends SimpleIOSuite with Checkers {
  ...
  test("successful checkout") {
    forall(gen) { case (uid, pid, oid, crt, crd) =>
      Checkout[IO](
...
protected class TestOrders() extends Orders[IO] {
  ...
  def get(userId: UserId, orderId: OrderId): IO[Option[Order]] = IO.none
...
protected class TestCart extends ShoppingCart[IO] {
  ...
  def get(userId: UserId): IO[CartTotal] = IO.pure(CartTotal(items = List.empty, Money(0.0, USD)))
