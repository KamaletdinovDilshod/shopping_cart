...
import shop.config.types.{AppConfig, PostgresSQLConfig, RedisConfig}
...
object AppResources {
  ...
  def make[
      F[_]: Concurrent: Console: Logger: MkHttpClient: MkRedis: Network
  ](cfg: AppConfig): Resource[F, AppResources[F]] = {
    ...
    def mkPostgresSqlResource(c: PostgresSQLConfig): SessionPool[F] =
      Session
        .pooled[F](
          host = c.host.value,
          port = c.port.value,
